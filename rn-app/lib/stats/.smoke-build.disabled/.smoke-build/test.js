...
async function main() {
    // 1. 累加播放 3 次(active)
    (0, index_1.recordPlayback)('video1', 5000, true);
    (0, index_1.recordPlayback)('video1', 3000, true);
    (0, index_1.recordPlayback)('video1', 2000, false); // background
    (0, index_1.recordShadowing)('video1');
    (0, index_1.recordShadowing)('video1');
    // 2. 累加另一个视频
    (0, index_1.recordPlayback)('video2', 10000, true);
    // 3. 强制 flush
    await (0, index_1.forceFlush)();
    // 4. 读回
    const v1 = await (0, index_1.getVideoStats)('video1');
    const v2 = await (0, index_1.getVideoStats)('video2');
    const all = await (0, index_1.getAllVideoStats)();
    const today = await (0, index_1.getDailyStats)(new Date().toISOString().slice(0, 10), new Date().toISOString().slice(0, 10));
    ...
    (0, index_1.recordPlayback)('video1', 5000, true); // 5s > 2s 阈值,应被忽略
    const v1After = await (0, index_1.getVideoStats)('video1');
    assert(v1After.foregroundMs === v1Before, 'seek delta 应被忽略');
    // 6. 测试负 delta
    (0, index_1.recordPlayback)('video1', -1000, true);
    const v1Neg = await (0, index_1.getVideoStats)('video1');
    assert(v1Neg.foregroundMs === v1Before, '负 delta 应被忽略');
    // 7. 测试空 videoId
    (0, index_1.recordPlayback)('', 1000, true); // 应被忽略,不抛错
